...
##fixed corruption of original data with BG generation
###################################################    
...
if gen_background == True:
    ...
    for run in range(0,num_bg):
        ...
        # FIX: Use .copy() to avoid corrupting original DataFrame data
        micro = micro_comp_df.values.copy()
        metab = metab_comp_df.values.copy()
...
##elbow method set num_metabolite_clusters = 4,5,6
num_metabolite_clusters = 4
...
binary_score_df.to_csv("results/" + out + "/CV/" + "/binary_score_matrix.csv")
...
## fixed numpy 2D array error in labels_df.values
################################################### 
...
###################################################
###export module network file as per jupyter notebook
## fixed error during .json export
###################################################
...
# Create metabolite cluster nodes
for sa in range(num_metabolite_clusters):
    node = {"data": {'id': f"Metabolic_{sa}"}}
    network["elements"]["nodes"].append(node)

# Create microbiome cluster nodes
for sa in range(num_microbiome_clusters):
    node = {"data": {'id': f"Microbiome_{sa}"}}
    network["elements"]["nodes"].append(node)

score_list = []

# Create edges with try-except for robustness
for mic in range(num_microbiome_clusters):
    print("\n\n")
    for met in range(num_metabolite_clusters):
        try:
            # Compute score
            values = binary_score_df.values[microbe_clusters == mic, :][:, metabolite_clusters == met]
            score = float(np.round(np.mean(values.flatten()), 3))
            
            # Create edge
            edge = {
                "data": {
                    "id": f"Micro_{mic}_to_Metab_{met}",
                    "source": f"Microbiome_{mic}",
                    "target": f"Metabolic_{met}",
                    "score": score
                }
            }
            ...
            print(f"Failed to compute edge Micro_{mic}_to_Metab_{met}: {e}")
            continue

# Save the network to JSON
try:
    with open(f"results/{out}/CV/network.json", 'w') as outfile:
        json.dump(network, outfile)
except Exception as e:
    print(f"Failed to write network file: {e}")
